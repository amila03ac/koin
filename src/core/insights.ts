// insights.ts — pure aggregation over normalized transactions. All spending math ignores
// `ignored` rows (internal transfers) and buckets by EFFECTIVE date (when the purchase
// actually happened). Date math is done in UTC to avoid timezone drift.
import type {
...
function toDate(isoDate: string): Date { return new Date(isoDate + "T00:00:00Z"); }
function iso(d: Date): string { return d.toISOString().slice(0, 10); }
function effDate(t: Transaction): string { return t.effectiveDate || t.postedDate; }
...
function periodLabel(anchorIso: string, period: Period): string {
  ...
  const f = (x: Date) => x.toLocaleDateString("en-AU", { day: "numeric", month: "short", timeZone: "UTC" });
  return `${f(start)} – ${f(end)}, ${end.getUTCFullYear()}`;
}

function shiftAnchor(anchorIso: string, period: Period, delta: number): string {
  const d = toDate(anchorIso);
...
function filterByPeriod(transactions: Transaction[], period: Period, anchorIso: string): Transaction[] {
  const key = periodKey(anchorIso, period);
  return transactions.filter((t) => periodKey(effDate(t), period) === key);
}

function availablePeriods(transactions: Transaction[], period: Period): string[] {
  const keys = new Set(transactions.map((t) => periodKey(effDate(t), period)));
  return [...keys].sort().reverse();
}

function spendingOnly(transactions: Transaction[]): Transaction[] {
  return transactions.filter((t) => !t.ignored);
}

function summary(transactions: Transaction[]): Summary {
...
function byCategory(transactions: Transaction[]): CategoryTotal[] {
...
function topMerchants(transactions: Transaction[], n?: number): MerchantTotal[] {
...
function biggestExpenses(transactions: Transaction[], n?: number): Transaction[] {
...
function trendBuckets(transactions: Transaction[], period: Period, anchorIso: string): TrendBucket[] {
