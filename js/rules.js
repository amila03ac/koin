...
// Pure functions attached to Koin.rules.
(function () {
  ...
  function matchesIgnore(description, ignorePatterns) {
    return (ignorePatterns || []).some((p) => {
      const re = toRegex(p);
      return re && re.test(description);
    });
  }

  function autoCategory(description, categoryRules) {
  ...
  function detectRecurringMerchants(transactions) {
  ...
  function apply(transactions, rules) {
    ...
    let out = transactions.map((t) => ({
      ...
      category: t.categorySource === "manual" ? t.category
  ...
  Koin.rules = { apply, matchesIgnore, autoCategory, detectRecurringMerchants };
})();
