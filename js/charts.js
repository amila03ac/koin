// charts.js — tiny, dependency-free inline-SVG charts. Attached to Koin.charts.
// Kept small so Koin needs no chart library and works offline.
(function () {
  ...
  // Vertical bars. data = [{ label, total }]. `opts.height` is the pixel height of the
  // TALLEST bar; every other bar is scaled to it from zero. Heights are set in pixels
  // (not %) and bars never flex-shrink, so proportions are always honest — a flex column
  // with % heights would let label text squeeze the tall bars and flatten the chart.
